...
class ViewController: UIViewController {
	...
	private func showOriginalStory(config: Config) {
		let animal = animalSegmentedControl.titleForSegment(at: animalSegmentedControl.selectedSegmentIndex)
		let story = """
	    At the age of \(config.age), \(config.name) took a trip to \(config.location) with \(Int(numberOfPetsStepper.value)) pets in order to \(config.verb) with a \(animal!). \(config.name) decided to buy \(Int(numberSlider.value)). Now they live happily ever after
	"""
